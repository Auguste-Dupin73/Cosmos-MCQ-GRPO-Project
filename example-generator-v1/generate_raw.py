...
_TIERC_REQUIRED_EVENT_VARS: Dict[str, Dict[str, set[str]]] = {
    ...
    "two_items_then_bundle_discount_total_cost": {
        ...
        # discount_pct is derived in reasoning:
        # subtotal = unit_price1*qty1 + unit_price2*qty2
        # final_total is given (not asked), so discount_pct can be solved.
        "discount_pct": {"unit_price1", "qty1", "unit_price2", "qty2", "final_total"},
    },
    "discounted_total_then_change": {
        "change": {"unit_price", "qty", "discount_pct", "paid"},
       "paid": {"discounted_total", "change"},
      # discounted_total is derived in reasoning from unit_price, qty, discount_pct
        # paid is solvable if change is given.
       "paid": {"unit_price", "qty", "discount_pct", "change"},
    },
    "transform_then_share": {
        "each": {"unit_price", "qty", "discount_pct", "people"},
       "people": {"discounted_total", "each"},
       # discounted_total is derived in reasoning from unit_price, qty, discount_pct
       # people is solvable if each is given.
      "people": {"unit_price", "qty", "discount_pct", "each"},
    },
...
def render_discounted_total_then_change(seed: DiscountedTotalThenChangeSeed, pb: Dict[str, Any], rng: random.Random, unknown_axis: bool, unknown_prob: float, stats: tuple[Counter, Counter] | None = None) -> str | None:
    ...
    discount_amount = gross_total * ctx["discount_pct"] // 100 
    ctx["gross_total"] = gross_total
    ctx["net_pct"] = net_pct
...
def render_transform_then_share(seed: TransformThenShareSeed, pb: Dict[str, Any], rng: random.Random, unknown_axis: bool, unknown_prob: float, stats: tuple[Counter, Counter] | None = None) -> str | None:
    ...
    if ask_target == "people":
        ctx["asked_people"] = ctx["discounted_total"] // ctx["each"]
...
def main() -> None:
    ...
    distractor_by_template = Counter()
    for i, seed in enumerate(seeds):
        for v in range(args.variants_per_seed):
            ...
            if isinstance(seed, ProduceConsumeSellSeed):
                ...
                if d_applied:
                    distractor_hits += 1
                    distractor_by_template[getattr(seed, 'template', 'unknown')] += 1
    ...
    if args.dump_stats:
        ...
        print("\n=== DISTRACTOR COUNTS BY TEMPLATE ===")
        for tpl, cnt in distractor_by_template.most_common():
            print(tpl, cnt)
