    ...
    for i, seed in enumerate(seeds):
        for v in range(args.variants_per_seed):
            ...
            if isinstance(seed, ProduceConsumeSellSeed):
                examples.append(render_produce_consume_sell(seed, pb, rng))
            elif isinstance(seed, RemainderAfterLossSeed):
                examples.append(render_remainder_after_loss(seed, pb, rng))
            elif isinstance(seed, EqualSharingSeed):
                examples.append(render_equal_sharing(seed, pb, rng))
            elif isinstance(seed, MultiStepAddSubSeed):
                examples.append(render_multi_step_add_sub(seed, pb, rng))
            elif isinstance(seed, UnitPriceQuantitySeed):
                examples.append(render_unit_price_quantity(seed, pb, rng))
            elif isinstance(seed, RateTimeSeed):
                examples.append(render_rate_time(seed, pb, rng))
            elif isinstance(seed, RatioScalingSeed):
                examples.append(render_ratio_scaling(seed, pb, rng))
            elif isinstance(seed, SumAndDifferenceSeed):
                examples.append(render_sum_and_difference(seed, pb, rng))
            elif isinstance(seed, CompareDifferenceSeed):
                examples.append(render_compare_difference(seed, pb, rng))
            elif isinstance(seed, ReverseOperationSeed):
                examples.append(render_reverse_operation(seed, pb, rng))
            elif isinstance(seed, BuyTwoItemsTotalCostSeed):
                examples.append(render_buy_two_items_total_cost(seed, pb, rng))
            elif isinstance(seed, ChangeFromPaymentSeed):
                examples.append(render_change_from_payment(seed, pb, rng))
            elif isinstance(seed, AddThenShareSeed):
                examples.append(render_add_then_share(seed, pb, rng))
            elif isinstance(seed, PercentageDiscountFinalPriceSeed):
                examples.append(render_percentage_discount_final_price(seed, pb, rng))
            elif isinstance(seed, PercentageIncreaseFinalPriceSeed):
                examples.append(render_percentage_increase_final_price(seed, pb, rng))
            elif isinstance(seed, DiscountedUnitTotalCostSeed):
                examples.append(render_discounted_unit_total_cost(seed, pb, rng))
            elif isinstance(seed, VatTotalCostSeed):
                examples.append(render_vat_total_cost(seed, pb, rng))
            elif isinstance(seed, BundleDiscountTotalCostSeed):
                examples.append(render_bundle_discount_total_cost(seed, pb, rng))
