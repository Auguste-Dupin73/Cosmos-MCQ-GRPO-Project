    ...
    for i, seed in enumerate(seeds):
        for v in range(args.variants_per_seed):
            ...
            if isinstance(seed, ProduceConsumeSellSeed):
                txt, applied = apply_intent_skin(render_produce_consume_sell(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, RemainderAfterLossSeed):
                txt, applied = apply_intent_skin(render_remainder_after_loss(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, EqualSharingSeed):
                txt, applied = apply_intent_skin(render_equal_sharing(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, MultiStepAddSubSeed):
                txt, applied = apply_intent_skin(render_multi_step_add_sub(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, UnitPriceQuantitySeed):
                txt, applied = apply_intent_skin(render_unit_price_quantity(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, RateTimeSeed):
                txt, applied = apply_intent_skin(render_rate_time(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, RatioScalingSeed):
                txt, applied = apply_intent_skin(render_ratio_scaling(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, SumAndDifferenceSeed):
                txt, applied = apply_intent_skin(render_sum_and_difference(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, CompareDifferenceSeed):
                txt, applied = apply_intent_skin(render_compare_difference(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, ReverseOperationSeed):
                txt, applied = apply_intent_skin(render_reverse_operation(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, BuyTwoItemsTotalCostSeed):
                txt, applied = apply_intent_skin(render_buy_two_items_total_cost(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, ChangeFromPaymentSeed):
                txt, applied = apply_intent_skin(render_change_from_payment(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, AddThenShareSeed):
                txt, applied = apply_intent_skin(render_add_then_share(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, PercentageDiscountFinalPriceSeed):
                txt, applied = apply_intent_skin(render_percentage_discount_final_price(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, PercentageIncreaseFinalPriceSeed):
                txt, applied = apply_intent_skin(render_percentage_increase_final_price(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, DiscountedUnitTotalCostSeed):
                txt, applied = apply_intent_skin(render_discounted_unit_total_cost(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, VatTotalCostSeed):
                txt, applied = apply_intent_skin(render_vat_total_cost(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
            elif isinstance(seed, BundleDiscountTotalCostSeed):
                txt, applied = apply_intent_skin(render_bundle_discount_total_cost(seed, pb, rng), rng, pb, args.intent_axis, args.intent_prob)
                if applied:
                    intent_hits += 1
                examples.append(txt)
                ...
                if out is not None:
                    txt, applied = apply_intent_skin(out, rng, pb, args.intent_axis, args.intent_prob)
                    if applied:
                        intent_hits += 1
                    examples.append(txt)
